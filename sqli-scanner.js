...
function parseArgs(argv) {
  const cfg = {
    ...
    maxPages: 200,
    maxDepth: 3,
    concurrency: 10,          // crawl concurrency (min enforced: 10)
    scanConcurrency: 10,      // scanning concurrency (min enforced: 10)
    timeout: 15000,
  ...
  for (let i = 2; i < argv.length; i++) {
    ...
    else if (a === "--concurrency") cfg.concurrency = parseInt(argv[++i] || "10", 10);
    else if (a === "--scan-concurrency") cfg.scanConcurrency = parseInt(argv[++i] || "10", 10);
  ...
  // Enforce minimum 10 threads for both phases
  cfg.concurrency = Math.max(10, cfg.concurrency|0);
  cfg.scanConcurrency = Math.max(10, cfg.scanConcurrency|0);
...
async function testOpenRedirect(base, ep, param, findings, log) {
  ...
  const values = [
    "https://example.com", // put your here
  ];
